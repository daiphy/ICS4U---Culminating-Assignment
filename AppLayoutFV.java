...
public class AppLayoutFV extends Application {
    ...
    public Scene showSceneOne(Stage stage) {
        ...
        // initialize the warning text if the user doesn't enter a csv file it'll change
        // in the gui
        ...
        Button newBudgetB = goToSceneTwo(window, "SELECT MONTH"); // When this button is clicked, it changes the scene
                                                                  // to scene 2
        newBudgetB.setPrefWidth(150);

        Button summaryB = goToSceneFive(window, "SUMMARY", warningOne); // When this button is clicked, it changes the
                                                                        // scene to scene 5
        summaryB.setPrefWidth(150);

        Button trendsB = goToSceneSeven(window, "TRENDS", warningOne); // When this button is clicked, it changes the
                                                                       // scene to scene 7
        ...
        importB.setOnAction(action -> {
            try {
                warningOne.setText(financeFV.checkInputtedFile(stage, this.month)); // Uses FinanceFV's method to check
                                                                                    // for correct file input
    ...
    public Scene showSceneTwo(Stage stage, ComboBox cBMonths) {
        ...
        // HBox gathers the month input and income categories input as they are in the
        // same row
        ...
        getMonth(cBMonths);
        showTextTwo(cBMonths, incomeCatT, expensesCatT, warningT, stage); // Displays the text that is already stored
        ...
        addExpCatB.setOnAction(action -> { // action for the add expense category button
            boolean check;
            check = onlyLetters(expCatTF.getText()); // INPUT VALIDATION - checks if its only letters and not
                                                     // symbols/numbers
        ...
        nextPageB.setOnAction(action -> { // action for going to next page button
            if (setMonth) { // if the user has clicked a month in the month combo box
                ...
                trends.incomeCatList = trends.defaultCategories(trends.incomeCatList, trends.defaultInc); // checks if the user's entries arraylist s empty, that means they want default categories
        ...
        middleScreen.getChildren().addAll(instructionsL, infoL, monthRow, incCatRow, expCatRow, catL, displayCat,
                warningT, sceneButtons);
    ...
    public Scene showSceneThreeFour(Stage stage, String whichType) {
        ...
        addTB.setOnAction(action -> {
            ...
            if (check && validTF) {
                warningT.setText("");
                showUserInput(true, true, cBInc, amntTFT, showIncCat, showIncAmt, showExpCat, showExpAmt, incomeCatArr,
                        incomeAmtArr);
            } else {
                warningT.setText("Please input a number. Do not include symbols or letters");
            }
        ...
        addBB.setOnAction(action -> {
            ...
            if (check && validTF) {
                warningT.setText("");
                showUserInput(true, false, cBExp, amntTFB, showIncCat, showIncAmt, showExpCat, showExpAmt,
                        expenseCatArr, expenseAmtArr);
            } else {
                warningT.setText("Please input a number. Do not include symbols or letters");
            }
        ...
        delTB.setOnAction(action -> {
            showUserInput(false, true, cBInc, amntTFT, showIncCat, showIncAmt, showExpCat, showExpAmt, incomeCatArr,
                    incomeAmtArr);

        });
        delBB.setOnAction(action -> {
            showUserInput(false, false, cBExp, amntTFB, showIncCat, showIncAmt, showExpCat, showExpAmt, expenseCatArr,
                    expenseAmtArr);
        });
        ...
        if (whichType.equalsIgnoreCase("Anticipated")) {
            ...
            nextB.setOnAction(action -> {
                trends.income2D = trends.populate(incomeCatArr, incomeAmtArr, trends.income2D, trends.incomeCatList, 1,
                        this.month);
                trends.expense2D = trends.populate(expenseCatArr, expenseAmtArr, trends.expense2D,
                        trends.expenseCatList, 1, this.month);
            ...
            Button nextB = features.yellowButton("NEXT");
            nextB.setOnAction(action -> {
                trends.income2D = trends.populate(incomeCatArr, incomeAmtArr, trends.income2D, trends.incomeCatList, 2,
                        this.month);
                trends.expense2D = trends.populate(expenseCatArr, expenseAmtArr, trends.expense2D,
                        trends.expenseCatList, 2, this.month);
        ...
        mainScreen.getChildren().addAll(titleHB, monthL, warningT, secondRow, thirdRow, stackPaneT, fourthRow, fifthRow,
                stackPaneB, lastRow);
    ...
    public Scene showSceneFive(Stage stage, ComboBox cBMonths) {
        ...
        if (this.month == null && clicked == false) {
            ...
            System.out.println(
                    "month is: " + this.month + " clicked is " + clicked + " imported is " + financeFV.fullImport);
        }
    ...
    public Scene showSceneSix(Stage scene, ComboBox cBMonths) {
        ...
        double endBalance = trends.sumCalculator(trends.income2D, 2) - trends.sumCalculator(trends.expense2D, 2); // Acc
                                                                                                                  // income
                                                                                                                  // -
                                                                                                                  // acc
                                                                                                                  // expense
        ...
        // If the acc income or end balance is a negative number, the default minimum
        // size for the rectangle (10) is used
        if (sumAccIncome < 0) {
        ...
        // If the acc income or end balance is greater than a certain value (100, 1500,
        // 10000, 100000),
        ...
        // summaryLeftStack.getChildren().addAll(new Rectangle(300, 150, babyBlue),
        // summaryLeftHB);
        ...
        // Label output depends on if the user gained (positive #) or lost money
        // (negative #)
        if (percentSaved > 0) {
        ...
        // If any of actual or anticipated income or expense is 0, a default rectangle
        // size (10) is used
        if (sumAntIncome < 0) {
        ...
        // If the values in anticpated and actual income or expenses are too big, all
        // the rectangle
        // sizes will be shrunk proportionately. For ex, when the values are greater
        // than 100, 1500, 10000, 100000
        ...
        allRect.getChildren().addAll(spacing, incomeSummaryVB, incAntAccValueHB, incRectVB, expenseSummaryVB,
                expAntAccValueHB, expRectVB, spacing2);
     ...
     * @return scene seven
     */
    public Scene showSceneSeven(Stage stage, ComboBox cBMonths) { // u have to click plan and transactions first because
                                                                  // thats what populates the 2d arr
        Scene seven;
    ...
    public ArrayList<String> arrUpdate(ArrayList<String> arrList, String[][] twoDArr, int col) {
        ...
        if (!startCoords.equals(":)")) {
            ...
            financeFV.repopulate(this.month, trends.expense2D, "expense");
            for (int i = 1; i < trends.income2D.length; i++) {
                arrList.add(twoDArr[i][col]); // populates the arraylist that we print out with what's already in the 2d
                                              // array
            }
    ...
    // for new budget
    public void showCategory(boolean add, ArrayList<String> categoryArrList, TextField catTF, boolean income,
            Text incomeCatT, Text expensesCatT) {
        String stringCat;
        String printCat = "";
        if (add == true) {
    ...
    public void showUserInput(boolean add, boolean income, ComboBox comboBox, TextField amntTF, Text catInc,
            Text amtInc, Text catExp, Text amtExp, ArrayList<String> catArr, ArrayList<String> amtArr) {
        // initialize strings
