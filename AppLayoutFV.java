...
public class AppLayoutFV extends Application{
  ...
  public Scene showSceneOne (Stage scene){
    ...
    //Title label, bees and instruction label
    ...
    //Buttons to scenes and button formatting
    ...
    importB.setOnAction(action ->{
      ...
      try {
        //Ff the user does not input .csv, the warning text will be shown
        warningT.setText(
  ...
  public Scene showSceneTwo (Stage stage, ComboBox cBMonths){
    ...
    //Set the chosen month to selected month from ComboBox
    getMonth(cBMonths);
    ...
    nextPageB.setOnAction(action->{
      ...
      sceneThree = showSceneThreeFour(window, "Anticipated"); //"Plan" scene
...
//-------------------- SCENE SIX BELOW --------------------//
  public Scene showSceneSix(Stage scene, ComboBox cBMonths){
    ...
    if (sumAccIncome < 0){
      sumAccIncomeR = new Rectangle (50, 10, babyBlue);
    }
    else{
   ...
   //Initiate labels to display calculation results
    ...
    if (sumAntIncome < 0){
      sumAntIncR = new Rectangle(10, 20, white); 
    }
    else{
  ...
  public Scene showSceneSeven(Stage stage){ // u have to click plan and transactions first because thats what populates the 2d arr
    ...
    //Gathers graphs and buttons to make the screem
    VBox mainScreen = new VBox(10);
